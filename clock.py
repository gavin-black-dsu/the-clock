...
• Temperature readout (top center) with independent brightness, color, and padding
• Anti burn-in drift
"""
...
def load_config(path):
    ...
    pad_top = int(data.get("temp_padding_top", PADDING))
    return (theme, b_day, b_night, b_sun, b_moon,
            b_temp_day, b_temp_night, col_day, col_night,
            pad_top)

THEME_NAME, B_DAY, B_NIGHT, B_SUN, B_MOON, B_TEMP_DAY, B_TEMP_NIGHT, TEMP_COL_DAY, TEMP_COL_NIGHT, TEMP_PAD_TOP = load_config(CONFIG_PATH)
...
TOP_RESERVED = max(PADDING + MAX_ICON_H, TEMP_PAD_TOP + TEMP_FONT_SIZE) + PADDING
...
while running:
    ...
    temp_y    = TEMP_PAD_TOP + temp_offset[1]
